...
class PatchNet(nn.Module):
    def __init__(self, args):
        ...
        self.convs_code_mult_hunks = nn.ModuleList([nn.Conv2d(Ci, Co, (K, Co * len(Ks))) for K in Ks])

        # other information
        self.dropout = nn.Dropout(args.dropout_keep_prob)
        self.fc1 = nn.Linear(3 * len(Ks) * Co, args.hidden_units)  # hidden units
    ...
    def forward_code(self, x, convs_line, convs_hunks, convs_multiple_hunks):
        ...
        # apply cnn 2d for each hunk in a commit code
        ...
        # apply cnn 2d for multiple hunks in a commit code
        x = x.reshape(n_batch, n_hunk, self.args.num_filters * len(self.args.filter_sizes))
        x = self.forward_msg(x=x, convs=convs_multiple_hunks)
        return x

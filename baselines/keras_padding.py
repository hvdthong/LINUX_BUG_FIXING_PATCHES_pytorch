...
def keras_extract_code(commits):
    codes = list()
    for c in commits:
        line = list()
        for t in c["code"]:
            diffs = t["diff"]
            for d in diffs:
                line += d.split(':')[3].strip().split(',')
        codes.append(" ".join(line))
    return codes


def padding_commit(commits, params):
    msgs, codes = extract_msg(commits=commits), keras_extract_code(commits=commits)
    msgs_codes = [m + ' ' + c for m, c in zip(msgs, codes)]
    dict_msg_code = dictionary(data=msgs_codes)

    # padding commit message
    pad_msg_code = mapping_commit_msg(msgs=msgs_codes, max_length=params.msg_length, dict_msg=dict_msg_code)

    labels = load_label_commits(commits=commits)
    return (labels, pad_msg_code, dict_msg_code)
